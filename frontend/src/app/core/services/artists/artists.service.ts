...
export class ArtistsService {
  ...
  getArtists(type: string = 'api'): Observable<ArtistResponse[]> {
    return this.http.get<ArtistResponse[]>(`${environment.backendUrl}/artists`);
  }

  getArtist(slug: string, type?: string): Observable<ArtistResponse> {
    return this.http.get<ArtistResponse>(
      `${environment.backendUrl}/artists/slug/${slug}`
    );
  }
}
